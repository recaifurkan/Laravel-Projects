    ...
    <div class="modal fade" id="forumKonu" tabindex="-100" role="dialog" aria-labelledby="exampleModalLabe2" aria-hidden="true">
        ...
        <div class="modal-body text-center">
            ...
            <div class="text-center" style="text-align: center; z-index: 10;">
                <p>Konu açmak için lütfen giriş yapınız...</p>
            ...
            <div class="form-group">
                    ...
                    <select name="kategori"  class="form-control kategori">
                            ...
                            @foreach ($selectKategoriler as $kategori)
                    <option value="{{$kategori->id}}">{{ilkHarfBuyuk($kategori->name)}}</option>
                     ...
                     <button id="kategoriButton" class="form-control">Kategorini Bulamadın mı? <span class="alert-danger">Tıkla</span></button>
                     <div style="display:none">
                        <input id="kategori_ad" name="kategori_ad" type="text" class="form-control" placeholder="Kategori isminizi buraya girebilirsiniz">
                       <div style="margin-top: 10px"> 
                            <div class="text-center" style="display: none"  id="kategori_ad_sonuc"  >
                    ...
                    @include('components.recaptcha',['forumId'=>'#konuEkleForum'])
                     <script>
                         ...
                         $(function(){
                            ...
                            $('#kategori_ad_sonuc').click(function(e){ // aşağı yazı çıkınca tıkladıpımzda altta önerdiğini gösteririr
                                ...
                                e.preventDefault();
                                // console.log('iki kere tıklandı');
                                $('#kategoriButton').next().slideUp(function(){
                                   $(this).prev().slideDown();
                               });
                            ...
                            $('#kategoriButton').click(function(e){
                             ...
                             $('#kategori_ad').on('input',function(e){
                                ...
                                var token = $("input[name='_token']").val();
                                 ...
                                 if(girilenYazi==''){
                                    ...
                                    $('#kategori_ad_sonuc').slideDown();
                                 
                                    options = $( ".kategori option" ); // optionsları topluyoruz
                                    select = $('.kategori'); // select formunu aldık
                                    
                                  
                                    $( ".kategori option" ).each(function(index,item){ //optionsları döndük
                                        
                                        var valuesi = select[0].options[index].value;// zaten 1 tane seleck olduğu için 0.cısını nseçtik
                                       
                                       eşleşme = item.text.toLowerCase().indexOf(girilenYazi.toLowerCase())// burada yazılan benziyo mu diye kontrol ediyoruz
                                   
                                        if (eşleşme>=0) // eğer eşleşme çok iyiyzs başka bişey yaptırmıyoz
                                            { 
                                               
                                                kategori = select[0].options[index].value;// selectin valuesini alıyoruz ve yukarıda kullanmak için eşleştiriyoruz
                                                // console.log(valuesi); 
                                                $('#kategori_ad_sonuc span').html('Benzer kategori '+' '+item.text+ '<span class="text-muted"> |2 tık|</span>');// altta çıkan yazıyla kullanıyoruz bu yazıyı
