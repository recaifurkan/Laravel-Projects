   ...
   <div class="card mb-4">
        <div class="card-body">
            ...
            <form id="searchForm" class="input-group" method="post" action="/konuSearch">
                @csrf
                <input id="search-text" name="searchText" type="text" class="form-control" placeholder="Konu Ara">
                <span class="input-group-btn">
                    <button value="searchSpot" name="searchKonu" class="btn btn-secondary" type="submit">Ara!</button>
                </span>
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if(typeof data !== 'undefined' && data.length > 0){
                                // console.log('boş');
                                Object.keys(data).forEach(function(konu , index) {
    
                                    text += '<div class="row mt-3">'+
                                                '<div class="forum-home-parent text-center col-12 pl-0">'+
                                                        '<div class="text-center col-md-12">'+
                                                            '<a style="height:20px;" class="text-center" href="'+'/forum/konu/' + data[konu]["url"] +'">'+ data[konu]["name"]  +'</a>'+
                                                            '</div> </div> <div class="hr"></div></div>';
    
        
                                     console.log(data[konu]['name']);
                              });
